...
import { resolveTechnicalScheduleLayout } from '@scli/domain';
import type {
  ...
  return (
    <Routes>
      <Route
        ...
        element={<ProjectLuminaireScheduleWorkspace />}
      />
    </Routes>
  );
}
...
const template = (
  ...
  imageSettings: { visible: true, width: 76 },
...
const output = (
  ...
  output: {
    ...
    locatorValue: `06_LUMINAIRE_SCHEDULE/REV_0${sequence}/schedule.${format.toLowerCase()}`,
    legacyAbsolutePath: null,
    contentHash: lifecycleState === 'FINALIZED' ? 'a'.repeat(64) : null,
    templateId: 'schedule.technical-modern',
    templateVersionId: 'v1',
    resolvedTemplateSnapshot: template(),
...
function outputPreview(body: {
  outputKind: 'LuminaireSchedule' | 'PresentationSchedule';
  format: 'PDF' | 'XLSX';
  templateId?: string;
  templateVersionId?: string;
  targetRevisionId?: string;
  options: { pageSize: 'A4' | 'A3'; orientation: 'Landscape'; productsPerPage?: 2 | 3 | 4 | null };
}): ResolvedOutputEnvelope {
  ...
  const selectedTemplate = template(
    body.templateId ?? 'schedule.technical-modern',
    body.templateVersionId ?? 'v2',
    body.templateId?.includes('consultant') ? 'Classic Grid Pro — Consultant' : 'Technical Modern',
  );
  if (body.templateId?.includes('consultant')) selectedTemplate.columns = columns.slice(0, 12);
  const resolvedTemplate = {
    ...selectedTemplate,
    family: body.outputKind,
    displayName:
      body.outputKind === 'PresentationSchedule' ? 'Presentation Schedule' : 'Technical Modern',
    versionId: body.templateVersionId ?? 'v2',
  };
  return {
    ...
    sourceFingerprint: body.templateId?.includes('consultant') ? 'd'.repeat(64) : 'a'.repeat(64),
    ...
    productsPerPage:
      body.outputKind === 'PresentationSchedule' ? (body.options.productsPerPage ?? 3) : null,
    messages: [{ code: 'OUTPUT_SUMMARY', level: 'INFO', message: '1 page, 2 luminaires.' }],
    pages:
      body.outputKind === 'PresentationSchedule'
        ? [{ kind: 'PresentationSchedule', pageNumber: 1, products: rows }]
        : [
            {
              kind: 'LuminaireSchedule',
              pageNumber: 1,
              layout: resolveTechnicalScheduleLayout(resolvedTemplate),
              rows,
            },
          ],
    rowCount: 2,
...
interface StubTarget {
  ...
  eligible: boolean;
  occupiedFormats?: Array<'XLSX' | 'PDF' | 'Both'>;
  rejection?: TechnicalOutputRequestedTargetView['rejection'];
  message?: string;
}

function stubApi(
  options: { ready?: boolean; rowCount?: number; target?: StubTarget; generateFails?: string } = {},
) {
  const classic = template(
    'schedule.classic-grid-pro.consultant',
    'v1',
    'Classic Grid Pro — Consultant',
  );
  classic.columns = columns.slice(0, 12);
  ...
  const fetch = vi.fn((url: string, init?: RequestInit) => {
    ...
    if (url.includes('/luminaire-schedule/generate') && init?.method === 'POST') {
    ...
    if (url.endsWith('/luminaire-schedule/config') && init?.method === 'PATCH') {
    ...
    if (url.includes('/luminaire-schedule')) {
      ...
      const requestedTarget: TechnicalOutputRequestedTargetView | null =
        requestedTargetId && target
          ? target.eligible
            ? {
                ...
                eligible: true,
                rejection: null,
                message: 'REV_06 can receive a generated Luminaire Schedule Output.',
          ...
          : null;
      const workspace: TechnicalScheduleWorkspaceView = {
        projectId: 'p1',
        currentRows,
        ...
        generationReadiness:
          options.ready === false
            ? { ready: false, reasons: ['Project folder required'] }
            : { ready: true, reasons: [] },
      };
  ...
  return { fetch, bodies, previewBodies };
}

function renderPage() {
  stubMatchMedia();
  renderV4(<V4Router />, ['/projects/p1/luminaire-schedule']);
}

/** Render the SAME route with a composition target carried in the address. */
function renderPageWithTarget(targetRevisionId: string = COMPOSED_TARGET_ID) {
  stubMatchMedia();
  renderV4(<V4Router />, [`/projects/p1/luminaire-schedule?targetRevisionId=${targetRevisionId}`]);
}

describe('ProjectLuminaireSchedulePage', () => {
  afterEach(() => {
    cleanup();
  ...
  it('wires the final reference toolbar to persisted settings and canonical output preview', async () => {
    const { bodies, previewBodies } = stubApi();
    stubMatchMedia();
    renderV4(<ProductionRouter />, ['/projects/p1/luminaire-schedule/advanced']);
    const main = await screen.findByTestId('v4-luminaire-schedule');
    expect(
      await within(main).findByRole('table', { name: 'Luminaire Schedule' }),
    ).toBeInTheDocument();
    expect(within(main).getByText('DL01')).toBeInTheDocument();
    fireEvent.click(within(main).getByRole('button', { name: 'Settings' }));
    fireEvent.change(within(main).getByRole('combobox', { name: 'Paper size' }), {
      target: { value: 'A3' },
    });
    await waitFor(() => expect(bodies).toContainEqual({ paperSize: 'A3' }));
    fireEvent.click(within(main).getByRole('button', { name: 'Close settings' }));
    fireEvent.click(within(main).getByRole('button', { name: 'Preview' }));
    expect(await screen.findByTestId('output-preview-paper')).toBeInTheDocument();
  ...
  it('opens the actual presentation output workflow from the final presentation tab', async () => {
    const { previewBodies } = stubApi();
    stubMatchMedia();
    renderV4(<ProductionRouter />, ['/projects/p1/luminaire-schedule/advanced']);
    const main = await screen.findByTestId('v4-luminaire-schedule');
    fireEvent.click(await within(main).findByRole('tab', { name: 'Presentation Schedule' }));
    fireEvent.click(within(main).getByRole('button', { name: 'Generate Output' }));
    expect(
      await screen.findByRole('dialog', { name: /Presentation Luminaire Schedule Preview/ }),
    ).toBeInTheDocument();
    expect(previewBodies[0]).toMatchObject({ outputKind: 'PresentationSchedule' });
  });

  it('keeps selected historical rows immutable and uses the canonical Revision identity', async () => {
    stubApi();
    stubMatchMedia();
    renderV4(<ProductionRouter />, ['/projects/p1/luminaire-schedule/advanced']);
    const main = await screen.findByTestId('v4-luminaire-schedule');
    fireEvent.click(await within(main).findByRole('button', { name: 'Schedule revision' }));
    fireEvent.click(screen.getByRole('option', { name: revision.revisionLabel }));
    await waitFor(() =>
      expect(within(main).getByText('Showing 1 to 1 of 1 luminaires')).toBeInTheDocument(),
    );
    expect(within(main).getByRole('button', { name: 'Generate Output' })).toBeDisabled();
    expect(within(main).getByRole('button', { name: 'Delete Revision' })).toBeDisabled();
    expect(within(main).getByRole('button', { name: 'Duplicate Revision' })).toBeEnabled();
  });

  it('renders the locked technical workspace from current canonical rows and effective template authority', async () => {
    stubApi();
    renderPage();
    expect(await screen.findByRole('heading', { name: 'Luminaire Schedule' })).toBeInTheDocument();
    await screen.findByText('DL01');
    expect(screen.getByRole('tab', { name: 'Technical Schedule' })).toHaveAttribute(
      'aria-selected',
      'true',
    );
    expect(screen.getByRole('button', { name: 'Preview Technical' })).toBeEnabled();
    expect(screen.getByRole('button', { name: 'Preview Presentation' })).toBeEnabled();
    expect(screen.getByRole('combobox', { name: 'Schedule template' })).toHaveDisplayValue(
      'Technical Modern · v1',
    );
    expect(screen.getByRole('columnheader', { name: 'Tag' })).toBeInTheDocument();
    expect(screen.getByText('DL01')).toBeInTheDocument();
    expect(screen.getByText('Showing 1 to 12 of 12 luminaires')).toBeInTheDocument();
    expect(screen.getByTestId('schedule-table-scroll')).toHaveClass('v4-schedule__table-scroll');
    expect(screen.getByTestId('schedule-workspace')).toContainElement(
      screen.getByTestId('schedule-main-card'),
    );
    expect(screen.getByTestId('schedule-workspace')).toContainElement(
      screen.getByTestId('schedule-inspector'),
    );
    expect(screen.getByTestId('schedule-main-card')).toContainElement(
      screen.getByTestId('schedule-pagination'),
    );
    const inspector = screen.getByRole('complementary', { name: 'Schedule Output Details' });
    expect(inspector).toHaveTextContent('Technical Modern');
  ...
  it('loads a canonical historical snapshot without changing the effective generation template', async () => {
    ...
    renderPage();
    await screen.findByText('DL01');
    fireEvent.change(screen.getByRole('combobox', { name: 'Schedule revision' }), {
    ...
    expect(screen.getByRole('button', { name: 'Preview Technical' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Preview Presentation' })).toBeDisabled();
    expect(screen.getAllByText('Technical Modern').length).toBeGreaterThan(0);
  });
  ...
  it('writes real template and paper-size overrides through the canonical config endpoint', async () => {
    ...
    renderPage();
    await screen.findByText('DL01');
    fireEvent.change(screen.getByRole('combobox', { name: 'Schedule template' }), {
      target: { value: 'schedule.classic-grid-pro.consultant::v1' },
    });
    await waitFor(() =>
    ...
    expect(
      fetch.mock.calls.some(
        ([url, init]) =>
          String(url).endsWith('/luminaire-schedule/config') && init?.method === 'PATCH',
      ),
    ).toBe(true);
  });
  ...
  it('persists column visibility, order, width and reset-to-template through canonical overrides', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Widen Wattage' }));
    await waitFor(() =>
      expect(bodies).toContainEqual({ columns: [{ columnId: 'wattage', width: 110 }] }),
    );
      ...
      expect(
        bodies.some(
          (body) =>
            ...
            body.columns.length === columns.length,
        ),
      ).toBe(true),
    );
  ...
  it('previews and generates only professional Schedule PDF/XLSX payloads', async () => {
    const { fetch, bodies } = stubApi();
    renderPage();
    await screen.findByText('DL01');
    fireEvent.click(screen.getByRole('button', { name: 'Preview Technical' }));
    await screen.findByRole('heading', { name: 'Technical Luminaire Schedule Preview' });
    fireEvent.change(screen.getByLabelText('Format'), { target: { value: 'XLSX' } });
    fireEvent.click(screen.getByRole('button', { name: 'Refresh Preview' }));
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Generate XLSX' })).toBeEnabled(),
    );
    ...
    expect(JSON.stringify(bodies)).not.toContain('TechnicalBoq');
    expect(JSON.stringify(bodies)).not.toContain('PresentationSchedule');
  });

  it('binds Preview template selection to the request and replaces the rendered layout', async () => {
    const { previewBodies } = stubApi();
    renderPage();
    await screen.findByText('DL01');
    fireEvent.click(screen.getByRole('button', { name: 'Preview Technical' }));
    const firstPaper = await screen.findByTestId('output-preview-paper');
    expect(firstPaper).toHaveAttribute('data-template-id', 'schedule.technical-modern');
    expect(previewBodies[0]).toMatchObject({
      templateId: 'schedule.technical-modern',
      templateVersionId: 'v1',
    });

    fireEvent.change(screen.getByRole('combobox', { name: 'Preview template' }), {
      target: { value: 'schedule.classic-grid-pro.consultant::v1' },
    });
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(screen.getByTestId('output-preview-paper')).toHaveAttribute(
        'data-template-id',
        'schedule.classic-grid-pro.consultant',
      ),
    );
  ...
  it('returns focus to the Preview launcher after Escape closes the workspace', async () => {
    stubApi();
    renderPage();
    await screen.findByText('DL01');
    const trigger = screen.getByRole('button', { name: 'Preview Technical' });
    trigger.focus();
    fireEvent.click(trigger);
    await screen.findByRole('dialog', { name: 'Technical Luminaire Schedule Preview' });
    fireEvent.keyDown(window, { key: 'Escape' });
    await waitFor(() =>
      expect(
        screen.queryByRole('dialog', { name: 'Technical Luminaire Schedule Preview' }),
  ...
  it('opens present artifacts, withholds Open for missing artifacts, and retries without duplicating history', async () => {
    ...
    expect(openPath).toHaveBeenCalledWith(
      'C:\\Projects\\TEST\\06_LUMINAIRE_SCHEDULE\\REV_05\\schedule.pdf',
    );
  ...
  it('blocks Golden generation with the real Project folder readiness reason', async () => {
    ...
    const generate = screen.getByRole('button', { name: 'Preview Technical' });
  ...
  it('uses shared pagination while retaining internal table scrolling', async () => {
    stubApi({ rowCount: 12 });
    renderPage();
    await screen.findByText('Showing 1 to 12 of 12 luminaires');
    fireEvent.change(screen.getByRole('combobox', { name: 'Rows per page' }), {
      target: { value: '10' },
    });
    expect(await screen.findByText('Showing 1 to 10 of 12 luminaires')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Next page' }));
    expect(await screen.findByText('Showing 11 to 12 of 12 luminaires')).toBeInTheDocument();
    expect(screen.getByTestId('schedule-table-scroll')).toHaveClass('v4-schedule__table-scroll');
  });
});
 ...
 * verdict is negative.
 */
describe('ProjectLuminaireSchedulePage — composition target (C1)', () => {
  afterEach(() => {
    cleanup();
    delete window.scliDesktop;
    cleanupV4();
  ...
  it('sends no targetRevisionId and shows no target chip when the address carries none', async () => {
    ...
    const reads = fetch.mock.calls
      .map(([url]) => String(url))
      .filter((url) => url.includes('/luminaire-schedule') && !url.includes('/generate'));
  ...
  it('surfaces occupied target slots before Preview generation', async () => {
    ...
    expect(await screen.findByTestId('v4-target-revision-meta')).toHaveTextContent(
      'Luminaire Schedule XLSX already present',
    );
    expect(screen.getByRole('button', { name: 'Preview Technical' })).toBeEnabled();
  });
  ...
  it('surfaces a server duplicate refusal verbatim and leaves the target intact', async () => {
    stubApi({
      ...
      generateFails: 'REV_06 already contains a Luminaire Schedule XLSX Output.',
    });
    renderPageWithTarget();
    await screen.findByText('DL01');
    await screen.findByTestId('v4-target-revision-chip');
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Generate XLSX' }));
    expect(
      await screen.findByText('REV_06 already contains a Luminaire Schedule XLSX Output.'),
    ).toBeInTheDocument();
