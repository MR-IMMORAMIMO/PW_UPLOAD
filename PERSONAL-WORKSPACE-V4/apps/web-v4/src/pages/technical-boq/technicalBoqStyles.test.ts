...
describe('Technical BOQ style contract', () => {
  ...
  it('contains wide columns inside the BOQ table surface and anchors pagination below it', () => {
    expect(css).toMatch(
  ...
  it('stacks the Inspector and toolbar at narrow widths without page-level horizontal overflow', () => {
    expect(css).toMatch(
      /@media \(max-width: 1500px\)[\s\S]*?\.v4-boq \.v4-schedule__toolbar\s*\{[^}]*grid-template-columns:\s*minmax\(180px, 1fr\) minmax\(175px, 0\.8fr\)/,
    );
    expect(css).toMatch(
    ...
    expect(css).toMatch(
      /@media \(max-width: 1080px\)[\s\S]*?\.v4-boq \.v4-schedule__toolbar\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/,
    );
    expect(css).toMatch(/\.v4-boq \.v4-schedule__toolbar-actions\s*\{[^}]*flex-wrap:\s*wrap/);
  });
});
