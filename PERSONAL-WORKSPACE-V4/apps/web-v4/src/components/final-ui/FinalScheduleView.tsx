...
import type { TechnicalScheduleWorkspaceView, OutputColumnDefinition } from '@scli/domain';
...
import {
  ...
  type ScheduleRow,
} from '../../pages/luminaire-schedule/luminaireScheduleViewModel';
export interface FinalScheduleBinding {
  workspace: TechnicalScheduleWorkspaceView;
  rows: ScheduleRow[];
  columns: OutputColumnDefinition[];
  selectedId: string | null;
  select: (id: string) => void;
  tab: 'technical' | 'presentation';
  setTab: (tab: 'technical' | 'presentation') => void;
  detailsOpen: boolean;
  close: () => void;
  template: string;
  setTemplate: (value: string) => void;
  revision: string;
  setRevision: (id: string) => void;
  page: number;
  pages: number;
  total: number;
  pageSize: number;
...
export default function FinalScheduleView({ binding: b }: { binding: FinalScheduleBinding }) {
  const activeTab = b.tab,
    setActiveTab = b.setTab,
    detailOpen = b.detailsOpen;
        ...
        >
          ...
          <div style={{ padding: '18px 20px 0' }}>
            ...
            <p style={{ fontSize: 12, color: C.textSub, margin: '0 0 14px' }}>
              Prepare and generate technical luminaire schedule with your preferred template.
            </p>

            {/* Tabs */}
            <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${C.border}` }}>
              {(['technical', 'presentation'] as const).map((tab) => (
                <button
                  key={tab}
                  role="tab"
                  ...
                  style={{
                    ...
                    borderBottom:
                      activeTab === tab ? `2px solid ${C.teal}` : '2px solid transparent',
                    marginBottom: -1,
                    fontFamily: "'SCT Final Inter', sans-serif",
                    whiteSpace: 'nowrap',
                  }}
                >
                  {tab === 'technical' ? 'Technical Schedule' : 'Presentation Schedule'}
                </button>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ marginLeft: 4 }}>
              ...
              <V4FilterSelect
                ...
                options={[
                  { value: '', label: 'Current / Live Schedule' },
          ...
          >
              ...
              >
                ...
                {b.rows.map((row) => (
                  <div
                    ...
                    style={{
                      ...
                      background:
                        b.selectedId === scheduleRowKey(row)
                  ...
                  >
                    <div role="cell" style={{ padding: '10px 6px' }}>
                      <input
                        ...
                        checked={b.selectedId === scheduleRowKey(row)}
                        onChange={() => b.select(scheduleRowKey(row))}
                ...
                {!b.total ? (
                  <p role="status" style={{ padding: 20, fontSize: 12, color: C.textSub }}>
                    No luminaires available in the selected schedule.
                  </p>
                ) : null}
              </div>
            </div>
            ...
            >
              <span style={{ fontSize: 12, color: C.textSub }}>
                ...
                {Math.min((b.page + 1) * b.pageSize, b.total)} of {b.total} luminaires
              </span>
              <div style={{ flex: 1 }} />
              <button
            ...
            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                {[
                  ...
                  ['Revision', revision?.revisionLabel ?? 'Current / Live Schedule'],
                  ['Paper Size', template.paperSize],
                  ['Orientation', template.orientation],
                  ['Units', 'Metric'],
                  ['Language', 'English'],
                  ['Header', template.headerSettings.visible ? 'Shown' : 'Hidden'],
                  ['Footer', template.footerSettings.visible ? 'Shown' : 'Hidden'],
                  ['Last Updated', formatScheduleDateTime(latest?.output.updatedAt ?? null)],
                ].map(([label, value]) => (
                  <div
                    key={label}
                    style={{
